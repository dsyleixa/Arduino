...
{ 
  char sbuf[128];   
  static   double   fLatt=0, fLong=0, fdegi, fminf, fmini, fsecf, fseci, fsecfrac, 
                    fLattold=0, fLongold=0, fLattmean, fLongmean, fdist=0, ETA=0.7;
  int16_t  intdeg; 
  int16_t  intmin, intsec;
  uint16_t dday, dmonth, dyear; 
  uint16_t dhour, dmin, dsec, dcsec, nsat;

  uint32_t Isecfrac;  // frac*10000
  ...
  if (gps.location.isValid())
  {
    ...
    fminf   = abs( 60*modf(fLattmean, &fdegi) );   // split: deg.xxxxx => fdeg + fminf(min.xxxxx  )
    ...
    Isecfrac= (uint32_t)(fsecfrac*10000);     
     
    databuf[4]=lowbyte(intdeg);                         // Latt
    databuf[5]=highbyte(intdeg);  
    databuf[6]=lowbyte(intmin);
    databuf[7]=lowbyte(intsec);
    databuf[8]=lowbyte(Isecfrac); 
    databuf[9]=highbyte(Isecfrac);
    databuf[10]=low3byte(Isecfrac);
    databuf[11]=high4byte(Isecfrac);
    
    sprintf(sbuf, "Latt: %+04d:%02d'%7.4f ", intdeg, intmin, fsecf);
    Serial.print(sbuf); 
    lcdprintxy(0, 50, sbuf);
    /*
    sprintf(sbuf, "Latt: %+04d:%02d'%02d.%-4d ", intdeg, intmin, intsec, Isecfrac);
    Serial.print(sbuf); 
    ...
    */
    ...
    databuf[12]=lowbyte(intdeg);                         // Latt
    databuf[13]=highbyte(intdeg);  
    databuf[14]=lowbyte(intmin);
    databuf[15]=lowbyte(intsec);
    databuf[16]=lowbyte(Isecfrac); 
    databuf[17]=highbyte(Isecfrac);
    databuf[18]=low3byte(Isecfrac);
    databuf[19]=high4byte(Isecfrac);
    
    sprintf(sbuf, "Long: %+04d:%02d'%7.4f ", intdeg, intmin, fsecf);
    Serial.print(sbuf); 
    lcdprintxy(0, 70, sbuf);
    /*
    sprintf(sbuf, "Long: %+04d:%02d'%02d.%-4d ", intdeg, intmin, intsec, Isecfrac);
    Serial.print(sbuf); 
  ...
  if (gps.time.isValid())
  {
    ...
    databuf[23]=lowbyte(dhour); 
    databuf[24]=lowbyte(dmin);
    databuf[25]=lowbyte(dsec);    
...
void sendData(){
  ...
  Wire.write(sendmsg, MSGSIZE); // send own 30 bytes back to master..
}
