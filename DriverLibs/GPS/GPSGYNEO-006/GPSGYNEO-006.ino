...
{ 
  char sbuf[128]; 
  static   double   fLati=0, fLong=0, fmin, fdecsec, 
                    fLatiold=0, fLongold=0, fLatimean, fLongmean, fdist=0, ETA=0.7;
  ...
  if (gps.location.isValid())
  {
    ...
    fLatimean = ETA*fLati + (1-ETA)*fLatiold;  // Lowpass-Filter
    fLongmean = ETA*fLong + (1-ETA)*fLongold; 
    
    fdist = dist ( fLatimean, fLatiold, fLongmean, fLongold );
    
    //.......... Latiitude ....................
    
    sprintf(sbuf, "Lati: %+012.7f " , fLatimean );     
    //Serial.print(sbuf);   
    //lcdprintxy(0, 20, sbuf);
   
    decdeg = (int)fLatimean;
    fmin   = ( fLatimean - (double)decdeg ) * 60;
    decmin = (int)(fmin);
    fdecsec= ( fmin - (double)decmin ) * 60 ;
    sprintf(sbuf, "Lati: %+04d:%02d'%7.4f ", decdeg, decmin, fdecsec);
    Serial.print(sbuf); 
    lcdprintxy(0, 20, sbuf);
