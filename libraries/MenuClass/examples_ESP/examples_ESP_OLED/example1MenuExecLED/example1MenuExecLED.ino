...
char * ExecList1(int menuline){  
  switch (menuline) {
     case  0:    ; break;
     case  1:    digitalWrite(LED_BUILTIN, !HIGH); // inverted ESP LED_BUILTIN logic
                 break;
     case  2:    digitalWrite(LED_BUILTIN, !LOW);  // inverted ESP LED_BUILTIN logic
                 break;   
     case  3:    ; break; 
     case  4:    ; break;
  }     
...
void loop() {
    ...
    if(line!=-1) {      // double click btnEnter: do if selected             
       // execute menu lines
       if( ID==1 )  {   // check for ID at 1 Menu level not required
           ExecList1(line);          
       }      
    }   
}
