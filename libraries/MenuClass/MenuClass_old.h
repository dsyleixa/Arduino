...
class tMenu {  
  ...
  public:      
     ...
     int32_t checkbtn(int8_t btop, int8_t bbtm, int8_t bfunc) {
        ...
        if(bfunc==2){                   // return 2 if already tagged 
           if (list[actln][len-1]=='>' 
            || list[actln][len-1]=='<'   
            || list[actln][len-1]=='!'   ) 
           {  
                 //tagged=-1;                 
                 return actln;    // '>','<','!' at end = immediate execute
           }           
           if(tagged==actln) {    // double click at previous tagged line
              return actln;       // return selected   
           }  
           else      
           if(tagged!=actln) {
               return -1;
        ...
        if(bfunc==3){                   // long press = tag/untag 
           if(tagged==actln) {          // untag (if tagged currently) 
              tagged=-1;
           }
           else  {             
                                     // '>','<','!' at end = immediate execute
              if (list[actln][len-1]=='>' 
               || list[actln][len-1]=='<'   
               || list[actln][len-1]=='!'   ) 
              {  
                 //tagged=-1;                 
                 return actln;     // '>' or '<' at end = return current line
              }       
              else tagged=actln;          // tag (if not tagged currently)                                                            
           }
           mdisplay();           
        }
        return -1;         
