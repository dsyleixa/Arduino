...
// history
// 48008: aktuelle Arbeitsversion
// 48007: syntaktisch korrekt,rechnerisch konsistent,Hash-Adr. gezielt stabilisiert
...
int         M=136,    // M=0x88
            ...
            turn=16,  // 16=Weiss, 8=Schwarz; turn^=24 wechselt
            Q,        // RootEval
            O,        // EPSQ
            K,
            R,
...
// J,Z=hashkeys;
{
   ...
   struct    HT*a=A + ((J + turn*E) & (U-1));     // Fix A
...
A:                                              // resume normal after best
               ...
               do {                                       // y traverses ray, or:
                  ...
                  {  v=p<6?board[x+8]-board[y+8]:0;                   // center positional pts.
                     ...
                     J+=KAB(y+0,board[y])-KAB(x+0,u)-KAB(H+0,t);
                     Z+=KAB(y+8,board[y])-KAB(x+8,u)-KAB(H+8,t)+G-S;    // update hash key
                     C=d-1-(d>5&p>2&!t&!h);
   ...
   while(CHESSRUN)
   {
      ...
      if(score!=15 && score!=-I && score!=0) {
         ...
         sprintf(sbuf,"  EPsq: %c%c (%3d)\n",
                 'a'+(O&7), '8'-(O>>4&7), O); Serial.print(sbuf);
