...

{
    ...
    // printf("Minimax q=%d l=%d e=%d E=%d z=%d n=%d\n", q, l, e, E, z, n);
    int j,
        r,
        m,
        v,
...
A:                                              // resume normal after best
                    ...
                    do {                                       // y traverses ray, or:
                        ...
                        if (d - !t > 1) {                         // remaining depth
                            ...
                            if (!(G & M)) {
                                board[F] = turn + 6, v += 50;    // castling: put R & score
                            }
...
C: if (m > I - M | m < M - I)d = 98;            // mate holds to any depth
    ...
    int res = m += m < e;                            // delayed-loss bonus
//  printf("> res=%d\n", res);
    return res;
}

static int rand_last = 0;
static int chess_rand()
{
...
RESTART:
    ...
    while (K--) {
        ...
        L = 8;
        while (L--) {
            board[16 * L + K + 8] = (K - 4) * (K - 4) + (L - 3.5) * (L - 3.5);    // center-pts table
        }
    }                                                             //(in unused half board[])
    N = 1035;
    while (N-- > M) {
        T[N] = chess_rand() >> 9;    // Hashtable ranom init
    }
