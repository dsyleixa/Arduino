...
// Code f�r Arduino Due


#define K(A,B) *(int*)(T+A+(B&8)+S*(B&7))

#define HTsize (1<<12) //   RAM:  Due: (1<<12) // PC: (1<<24) // Arduino Mega  1<<8 ! 
...
#define MAXNODES 80000   //  wegen Zugdauer; für PC: x10 = 800000 = 8e5   // <<<<<<  fÃ¼r Mega verkleinern auf 10000! 
...
int  Minimax(int32_t  alpha, int32_t  beta, int32_t  score, int  EPC, int  prev, int32_t  depth)         
                       /* (alpha,beta)=window, score, EnPass_sqr.*/
                       /* prev=prev.dest; HkJ,HkZ=hashkeys; return score*/
{
   int  j,
        r,
        m,
   ...
   struct HTab *a = HTarray + (HkJ + turn * EPC & HTsize-1);   /* lookup pos. in hash table*/
   ...
   if(a->K-HkZ|prev|                                  /* miss: other pos. or empty*/
     ...
     )                                          /* time's up: go do best    */
   {
      ...
      P=d<3 ? I : Minimax(-beta,1-beta,-score,S,0,d-3);               /* Search null move         */
      m = (-P<beta | R>35) ? ( d>2 ? -I : score ) : -P;            /* Prune or stand-pat       */
...
labelA:                                        /* resume normal after best */
               ...
               {                                       /* y traverses ray, or:     */
                  ...
                  if(d-!t>1)                               /* remaining depth          */
                  {
                     ...
                     HkJ+=K(y+0,board[y])-K(x+0,upiece)-K(H+0,t);
                     HkZ+=K(y+8,board[y])-K(x+8,upiece)-K(H+8,t)+G -S;  /* update hash key          */
                     C=d-1-(d>5&p>2&!t&!h);
