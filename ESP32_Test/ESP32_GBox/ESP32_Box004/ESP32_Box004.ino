...
//       bottom=350-352
//=====================================================================
static int    actualfilenr=-1, selectfilenr=-1;
...
void ls() {
   ...
   for (int cnt = 0; cnt < flcnt; cnt++) {
      ...
      if(cnt==selectfilenr) COLOR_TEXT=YELLOW;
...
void showScreenFileMenu() {
   if (isInRange( adc00, 346, 356 ) ) {     // btn bottom/esc
      ...
      if(FILELISTACTIVE) {
         ...
         ls();
         markPos(actualfilenr, actualfilenr);
      }
   }
   else                                     // btn dn
      if (isInRange( adc00, 162, 172 ) && FILELISTACTIVE && actualfilenr <= flcnt) {
         if (actualfilenr < flcnt-1) {
            markPos(actualfilenr, actualfilenr+1);
            actualfilenr++;
         }
         delay(1);
         adc00 = ads0.readADC_SingleEnded(0);  // ADS1115 port A0
         ...
         if (isInRange( adc00, 0, 10 ) && FILELISTACTIVE && actualfilenr >= 0) {
            markPos(actualfilenr, actualfilenr-1);
            if (actualfilenr >= 0) actualfilenr--;
            delay(1);
            adc00 = ads0.readADC_SingleEnded(0);  // ADS1115 port A0
            delay(1);
            adc00 = mapConstrain(adc00);
   ...
   if(actualfilenr>=0) {
      if (FILELISTACTIVE) fileMarked=filelist[actualfilenr];
      else fileMarked="";
   }

   // btn right
   if (isInRange( adc00, 29, 39 ) && FILELISTACTIVE && actualfilenr >= 0) {
      if(selectfilenr!=actualfilenr) {
         selectfilenr=actualfilenr;
         fileSelected=filelist[selectfilenr];
      }
      else if(selectfilenr==actualfilenr) {
         selectfilenr= -1;
         fileSelected= "";
      }
      delay(1);
