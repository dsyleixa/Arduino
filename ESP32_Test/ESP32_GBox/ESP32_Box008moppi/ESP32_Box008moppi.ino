...
void showScreenFileMenu() {
   ...
   if (TFTMODE==1) {                                 // (TFTMODE==1), btn dn
      ...
      if (isInRange( adc00, 162, 172 ) &&  cursorfilenr <= filecount) {
      ...
      else if (isInRange( adc00, 0, 10 ) && cursorfilenr >= 0) { // btn up
         markPos(cursorfilenr, cursorfilenr-1);
      ...
      // btn right
      if (isInRange( adc00, 29, 39 ) && cursorfilenr >= 0) {
...
void setup() {
   ...
   server.on( "/", []()    {
      // Send html script to browser
      script  = " <!DOCTYPE html >";
      script += "Website Test ";
      script += ("<br> \n\n");
      script += "WiFi.localIP() : ";
      script += ("<br> \n\n");
      script += WiFi.localIP().toString();
      script += " </html>";
   ...
   display.setTextColor(WHITE);
   display.setCursor(0, tftline); display.print("Web server started!");
   tftline+=15;
