...
// change log:
// 1.8: racket pos offset a: speed+reflex fixes
   ...
   while(PONGACTIVE) {
      ...
      */  
      ...
      if(Bally>MAXDISY-BALLRAD-1) {
         ...
         // racket hit
         // left Racket
         if( Ballx>=RECOFFS-BALLRAD  && Ballx<=RECOFFS+RECWIDX+BALLRAD
               && Bally>=lRposy && Bally<=lRposy+RECHIGH )
         {
            ...
            // right Racket
            if( Ballx>=MAXDISX-RECOFFS-RECWIDX-BALLRAD  && Ballx<=MAXDISX-RECOFFS+BALLRAD
