...
// change log:
// 2.9: chess GUI ts moves (based 2.6.g) a:rework illegal
...
RESTART:
   display.fillRect(0, 0, 140,320, BLACK);
   display.setTextColor(WHITE);
   K=8;
   turn=16;
   ...
   while(CHESSACTIVE && CHESSFIGHT>0) {
      ...
      if (!Serial.available()) {
         while(z<25 ) {
            ...
            if(tsBtnPressed==4) { // TsBtnDown
               score=I;
               do {
                  tsBtnPressed = GetTSbuttons();
                  delay(1);
               } while(tsBtnPressed>=0);
               tsBtnPressed=-1;
               goto  MOVE_STRING;
            }
            tsBtnPressed = GetTSbuttons();
...
MOVE_STRING:
         ...
         Serial.println("checkscore="+(String)checkscore);
         ...
         if(score!=I ) {  // WTF...!?!?
            ...
            display.print ((String)"illegal? ");
         }


         //else if(score!=15 && score!=-59 && score!=-I &&  score!=0)
         {  // not check/stalemate
            RemP=S;
