...
int GetTSbuttons() {
   ...
   tsx = p.x = map(p.x, TS_MAXX, TS_MINX, 0, display.height());
   tsy = p.y = map(p.y, TS_MINY, TS_MAXY, 0, display.width());
   tsz = p.z;
...
anew:
   ...
   display.fillRect(BOXSIZE*7, 0, BOXSIZE, BOXSIZE, HX8357_WHITE);
   display.fillRect(BOXSIZE*8, 0, BOXSIZE, BOXSIZE, currentcolor);
   display.drawRect(2+BOXSIZE*8, 2, BOXSIZE-4, BOXSIZE-4, HX8357_WHITE);
   ...
   while (PAINTACTIVE) {
      ...
      y=tsx; x=tsy; z=tsz; // revert for orientation=3!
