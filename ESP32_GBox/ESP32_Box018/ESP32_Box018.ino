...
int Pong() {
   ...
   while(PONGACTIVE) {
      ...
      else if(adc02<450) lRvy= lRvy +dvtemp;
      else {
         lRvy=0;   // Rack middle=525
      }
      ...
      if(Bally>MAXDISY-BALLRAD-1) {
         ...
         // racket hit
         // left Racket
         if( Ballx>=RECOFFS  && Ballx<=RECOFFS+RECWIDX+BALLRAD
               && Bally>=lRposy-1 && Bally<=lRposy+RECHIGH+1 )
         {
            Ballx=RECOFFS+RECWIDX+BALLRAD;
            Bvy=Bvy + (lRvy/3);
            Bvx=-Bvx;
         }
         else if(Ballx<=BALLRAD)  { // debug: in Game => to off
            BallOff=true;
            ScoreR+=1;
            ...
            // right Racket
            if( Ballx<=MAXDISX-RECOFFS  && Ballx>=MAXDISX-RECOFFS-RECWIDX-BALLRAD
                  && Bally>=rRposy-1 && Bally<=rRposy+RECHIGH+1 )
            {
...
void setup() {
   ...
   // TS buttons
   //
   TSbutton1.initButton(&display, display.width()-30, 20,  60,30,  CYAN, BLUE, YELLOW, "1 ^", 2);
   TSbutton2.initButton(&display, display.width()-30,100,  60,30,  CYAN, BLUE, YELLOW, "2 <", 2);
   TSbutton3.initButton(&display, display.width()-30,180,  60,30,  CYAN, BLUE, YELLOW, "3 >", 2);
   TSbutton4.initButton(&display, display.width()-30,260,  60,30,  CYAN, BLUE, YELLOW, "4 v", 2);
