...
signed char    psymbol[]= ":?+nkbrq?P?NKBRQ";
   ...
   {  x=B=X;                                        // start scan at prev. best
      ...
      do {
         ...
         {  r=p=u&7;                                    // p = piece type (set r>0)
            ...
            {
A_CHESS:                                              // resume normal after best
               ...
               do {                                       // y traverses ray, or:
                  ...
                  m=i<0?I:m;                                // K capture
                  if(m>=l&d>1)goto C_CHESS;                 // abort on fail high
...
C_CHESS:
      ...
      // if(z) sprintf(sbuf, "%2d ply, %9d searched, score=%6d by %c%c%c%c\n",
      ...
      if(z) {
         ...
         Serial.print(sbuf);
         sprintf(sbuf, "\n%2d ply, %9d searched (%c%c%c%c)",d-1,N-S,
                 'a'+(X&7),'8'-(X>>4),'a'+(Y&7),'8'-(Y>>4&7));
...
void Chess() {
   ...
   int sqr, col;
   // int xoffs=160, yoffs=18, frx=16, fry=15; // GUI piece offset, GUI boardframe offset
   ...
   /*
      // debug
      Serial.print  ((String)0+"  ="+GetSqrColor(0)+"  ");  // A8
      Serial.println((String)1+"  ="+GetSqrColor(1)+"  ");  // B8
      Serial.print  ((String)118+"="+GetSqrColor(118)+"  ");  // G1
      Serial.println((String)119+"="+GetSqrColor(119)+"  ");  // H1
   */
...
RESTART:
   display.fillRect(0, 0, 140,40, BLACK);
   K=8;
   turn=16;
   ...
   while(CHESSACTIVE && CHESSFIGHT>0) {
      ...
      //display.print(sbuf);
      Serial.println();
      if(turn==16) sprintf(sbuf,"> WHITE: ");
      else sprintf(sbuf,">  BLACK:  ");
      Serial.print(sbuf);
      display.fillRect(0, 260, 140,20, BLACK);
      ...
      if (!Serial.available()) {
         while(z<25 ) {
            ...
            if(tsBtnPressed==3) { // TsBtnRight
               strcpy(cstring,"");
               goto  MOVE_STRING;
            }
            tsBtnPressed = GetTSbuttons();
            delay(1);
            x=tsx; y=tsy-10; z=tsz; // transformed ts coords to local var; tsy corr!
      ...
      if(x>=xoffs && y>=yoffs && x<xoffs+237  && y<yoffs+237 && z>30) {
         TSMOVESTATE=1;
         strcpy(pfrom, "");
         strcpy(pto, "");
         ...
         while(z>30) {
            ...
            display.fillRect(160,280, 280,40, BLACK);  // x1,y1,...
            display.setTextColor(WHITE);
            Serial.println(sbuf);
      ...
      else if(cstring[0]=='R' && strlen(cstring)<=1 ) {
         Serial.print("\n> RESET/RESTART \n");
         display.print("\n> RESET/RESTART \n");
         goto RESTART;
      }
...
MOVE_STRING:
         ...
         if(cstring[0]!=0 && strlen(cstring)>=4) {                                   /* parse entered move */
            K= cstring[0]-16*cstring[1]+799;
            L= cstring[2]-16*cstring[3]+799;
         }
         else
            strcpy(cstring, ""); // input error
         ...
         Serial.print ((String)"\nscore="+score );
         display.print((String)"score="+score );
         ...
         else if(score==15 || score==-59) {
            Serial.print("\n\nILLEGAL!\n");
         }
         if(score==-1234) {
            Serial.print("\n\nIN CHECK!\n");
         }
         else if(score!=15 && score!=-59 && score!=-I &&  score!=0) { // not check/stalemate
            RemP=S;
            ...
            display.fillRect(0,0, 140,40, BLACK);
            //display.fillRect(0,0, 140,240, BLACK); // left-hand GUI board field
            display.setCursor( 0, 0 );
...
void showAppsMenu() {
   ...
   if(MENULEVEL==1) {
      if(newMenuLevel) {
         ...
         display.println("      (1: EX)      "); display.setTextColor(BLUE);   display.println();
         display.println("      (2: Up)      "); display.setTextColor(LIME);   display.println();
         display.print  ("  (3: Le)");            display.setTextColor(RED);
         display.println(          " (Ri/OK)"); display.setTextColor(YELLOW); display.println();
