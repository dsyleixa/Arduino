...
// change log:
// B31x= B31w rollback Sicherung, undo ok
// B31w undo: Teile 2-6 gecheckt: Zugnummern sind Halbschritte, undo ok
// B31v= B31u rollback Sicherung
// B31u undo: Teil 5:  falsche Anzeige des letzten undo
//   (immer 2 Halbzüge statt nur 1, voriger Zug noch gelistet, unten Nr./turn falsch)
// B31t undo: Teil 5: UNDOcount+oldmovecount gefixt,  aber zu weit unter Zug 1.0
// B31s undo: Teil 4: UNDO nutzt Board-Snapshot, aber noch undo nur 1x
// B31r undo: Teil 5: mvh.board + .turn zuweisen (2 Zeilen), neue Teile-Nummerierung
// B31q undo: struc mvh erweitert mvh.board + .turn, noch falsche Teile-Nummerierung
// B31p undo per Erweiterung von mvh: neue Aufteilung (noch falsch)
// B31o2 Code-Rückzugssicherung  (mvh unverändert)
// B31o neue Aufteilung von Chess. 1-5, neue Stack-Funktionen deklariert (mvh unverändert)
// B31n test zus. Fixes, bislang stabil (1 undo)
// B31m läuft mit SerMon+GUI
// B31l chess minimax fix, Steuerung per TS 4=Dn: Illegal, aber Ser. ENTER: OK
//      Stelle: if(tsBtnPressed==4) { // TsBtnDown=auto  Z.1735
// 3.1k port ford 202 8008 8081
...
void read_ADS0() {
   static int valmax=17700; // 17300 17400 17500 17600 17700
...
int32_t btnstate00(int val) {
   if (isInRange( adc00,  PAD0_NONE, 1005 ) ) return 0; // NONE
   if (isInRange( adc00,  PAD0_ESC,   500 ) ) return 1; // ESC
   if (isInRange( adc00,  PAD0_UP,    220 ) ) return 2; // up
   if (isInRange( adc00,  PAD0_LEFT,   36 ) ) return 3; // left
   if (isInRange( adc00,  PAD0_RIGHT,  90 ) ) return 4; // right
   if (isInRange( adc00,  PAD0_DOWN, 1024 ) ) return 5; // down
   if (isInRange( adc00,  0,   3 ) )         return 5; // down
}
...
int WIFIPORT = 8008;  // ford 202
int WEBPORT =  8081;  // ford 202
...
void Pong() {
   ...
   while(PONGACTIVE) {
      ...
      if ( /*adc01>=400 || */ btn00==1) {    // AD-Button: ESC
...
// Chess Programm nach H. g. Muller micromax 4.8
//===========================================================
// ver 31w
...
//int WHITEPIECECOL = WHITE; // WHITE;
//int WHITEPIECECOL = LIGHTBLUE; // WHITE;  // LIGHTBLUE;
int WHITEPIECECOL = BLUE; // WHITE;  // BLUE;
//int WHITEPIECECOL = RED; // WHITE;  //RED;
//int WHITEPIECECOL = GREEN; // WHITE;  //GREEN;
...
//===========================================================
// Micromax : minimax Move Generator
//===========================================================
...
int      tsBtnPressed=-1;
uint32_t ChessAIStartMillis;;

#define KAB(A,B) *(int*)(T+A+(B&8)+S*(B&7))

#define JA(A) KAB(y+A,board[y])-KAB(x+A,u)-KAB(H+A,t)

#define U (1<<10)        // (1<<9) // (1<<10) // (1<<11) // <<<<< test!
...
int   M=136,        /* M=0x88                */
      ...
      Z,        // hashHi 
      turn=16,  
      //savturn=16 // veraltet, jetzt in mvh
      oldturn=16;
...
#define UNDO_MAX    5  // (undo move history limit)
#define MVLIST_MAX 12  // Listenlänge für vergangene Züge
...
int Minimax(int32_t q, int32_t l, int32_t e, int E, int z, int32_t n)  /* recursive minimax search, k=moving side, n=depth*/
// (q,l)=window, e=curr.eval.score, E=e.p.sqr., z=prev.dest; n=depth; return score
// J,Z=hashkeys;
{
   int  j,r,m,v,d,h,i,F,G,V,P,f=J,g=Z,C,s;
   // char  t,p,u,x,y,X,Y,H,B;
   uint8_t  t,p,u,x,y,X,Y,H,B;   // Fix B

   // struct HT_ *a=A+(J+turn*E&U-1);                 /* lookup pos. in hash table*/
   struct    HT_ *a = A + ((J + turn*E) & (U-1));     // Fix A
   ...
   {
      if( STOPTHINK && turn==oldturn && (millis()-ChessAIStartMillis>=7000) ) break;
            ...
            {  A:                                        /* resume normal after best */
               ...
               do {                                      /* y traverses ray, or:     */
                  ...
                  {  v=p<6?board[x+8]-board[y+8]:0;                  /* center positional pts.   */
                     ...
                     J+=JA(0); Z+=JA(8)+G-S;                   /* update hash key          */
                     C=d-1-(d>5&p>2&!t&!h);
...
C:
      ...
      if(z) {
         ...
         if(busycount==0) {
            display.fillRect(0,300, 420,20, BLACK);// letzte Zeile unten komplett
...
/*
   wieso ist if(idx < 2) idx = 2; // untere Grenze Startstellung ?
   Zugnummern sind IMMER Halbschritte!!
   1.0 1.5 2.0 2.5 ...
   die Startstellung ist 1.0 (WHITE) :
   zurückgenommen werden kann also, sobald WHITE versehentlich einen falschen Zug gemacht hat
   (d2-d4 oder was auch immer),
   dann wird sofort angezeigt: 1.5 BLACK>
   bevor BLACK einen eigenen Zug gestartet hat.
   Wird jetzt also 'U' eigegeben, ist die aktuelle Zugnummer 1.5,
   diese muss um 0.5 auf 1.0 zurückgesetzt werden,
   und der vorigen Zug von WHITE d2-d4 muss ebenfalls zurückgesetzt werden
   (Bauer/Pawn zurück auf d2),
   danach dann also Anzeige 1.0: // leer
   .
   .
   .
   //(unten:) 1.0 WHITE>
   :
   Also erst bei Zugnummer 1.0 ist kein weiteres 'U' mehr zulässig,
   bei zugummer >=1.5 DARF zurückgesetzt werden!
...
typedef struct {
   float        c;              // movecount (Halbschritt)
   char         piece[3];
   char         sfrom[5];
   char         sto[5];
   char         sb[3];
   signed char  board[129];     // <<< NEU: Board-Snapshot
   signed char  turn;           // <<< NEU
} MvHist;
...
void listMovHist(int l, int j) {  // letzte Züge, seitlich vom GUI Board
   char spiece;
   for(int k=l; k<=l+MVLIST_MAX && k<=j; k++) {
...
void Chess() {
   ...
   MaxMvh = UNDO_MAX;  // start init, minimum size 5
   ...
   //-----------------------------------------------------------
   // CHESS GUI Board settings
   //-----------------------------------------------------------

   //int sqr, col; << outcomment 5.2.26 031w
   ...
   display_mutex.lock();
   display.fillScreen(BLACK);
   // GUI Board: int xoffs=160,yoffs=18(1st sqr), frx=15, fry=15(frame);
   // Fläche: ab (x=143,y=1),  x2=143+270=313, h=270+1=271
...
RESTART:
   display.fillRect(0, 0, 140,240, BLACK); //Hochkant-Rechteck li, unten 80frei
   display.setCursor( 0, 0 );  // für move history (Halbzüge)
   oldmovecount=movecount=1;

   if(mvh) {       // free old mvh heap
      free(mvh);   //Fix B
      mvh = NULL;
   }
   MaxMvh = UNDO_MAX;  // start init, minimum size 5
...
UNDO:
   // verkürztes init, falls bereits Züge existieren
   display.setTextColor(WHITE);
   CHESSFIGHT = 1;
   ...
   while(CHESSACTIVE && CHESSFIGHT) {
      ...
      while(++N<121) {
         if(N&8 && (N+7!=0) ) {           // 1...8 board right
            ...
            int side=-1; // side aka turn
            if( (spiece>='A' && spiece<='Z') || spiece=='*') side=16;
            else if( (spiece>='a' && spiece<='z') || spiece=='+') side=8;

            // paint board
            PaintBlankSqr(N);
            if(side>1) Piece2Sqr( GetPieceBmp(spiece), side, N); // add piece ico
         }
      }

      STOPTHINK=0;
      MAXDEEP = CUSTDEEP;  //   shorten think time //Fix C

      sprintf(sbuf,"  --------------- \n  A B C D E F G H ");
      Serial.print(sbuf);
      //display.print(sbuf);
      Serial.println();
      if(turn==16) sprintf(sbuf,"%4.1f WHITE> ", movecount);  // sprintf Statuszeile
      else sprintf(sbuf,"%4.1f BLACK> ", movecount);
      Serial.print(sbuf);                            //  Monitor print Statuszeile

      display.fillRect(0, 240, 140,20, BLACK);       // frame vorletze Zeile unten, 30%breit
      display.setCursor(0,240);
      display.print(sbuf);                           // GUI print Statuszeile

      display.setCursor(426, 87);       // GUI ts btn Interface
      display.print("new"); // 1
      display.setCursor(426,140);
      display.print("undo"); // 2
      display.setCursor(426,193);
      display.print("   ");  // 3
      display.setCursor(426,245);
      display.print("auto"); // 4

      // move list
      display.fillRect(0, 0, 140,240, BLACK);// Hochkant-Rechteck li, unten 80frei
      display.setCursor( 0, 0 );
      int m = ( (int)(oldmovecount*2) );
      int l = max(2,m-MVLIST_MAX);
      listMovHist(l,m);             // für listMovHist: letzte MVLIST_MAX=12 Halbzüge
      ...
      tsBtnPressed = GetTSbuttons();
      x=tsx; y=tsy; z=tsz; // transformes ts coords to local var
      ...
      else if(tsBtnPressed==2) { // TsBtnDLeft=UNDO <<<<<<<<<<<<<<<< 'U'
         strcpy(cstring,"U");
         score=I;
         i=1;
         tsBtnPressed=-1;
      ...
      if (!Serial.available()) {
         while(z<20 ) {
            ...
            read_ADS0(); // read ads for 5-Btn-Pad

            if (btn00==1) {       // AD-Btn-ESC=QUIT
               goto QUIT_CHESS;
            }
            if (btn00==3) {      // AD-Btn3-Left=UNDO <<<<<<<<<<<<<<<< 'U'
               strcpy(cstring,"U");
               goto EVAL_STRING;
            }
            if (btn00==5) {      // AD-Btn1-Dn=auto ("")
               strcpy(cstring,"");
               goto EVAL_STRING;
            }
            btn00=0;   // korrekt? <<<<<<<<<<< beobachten!
      ...
      if(x>=xoffs-20 && y>=yoffs && x<xoffs+237  && y<MAXDISPY && z>30 ) {
         TSMOVESTATE=1;
         strcpy(pfrom, "");
         strcpy(pto, "");
         strcpy(cstring, "");
         ...
         while( z>20 ) {
            ...
            display.fillRect(140,280, 280,40, BLACK); //untere 2Z. um 140 versetzt, dann volle Breite
            Serial.println(sbuf);
...
// es folgt Label EVAL_STRING:
// <<<<   neu  031w

//--------------------- Ende  Teil 4 ------------------------

//-------------------- Anfang  Teil 5 -----------------------
// >>>>  PATCH 031y  (Single-Halbzug-Undo korrekt)
...
EVAL_STRING:
      ...
      else if(cstring[0]=='U' && strlen(cstring)<4 ) {
         score = I;

         // --------- KORREKTES SINGLE-HALBZUG-UNDO ---------

         if(movecount <= 1.0) {
            // Startstellung – kein weiteres Undo erlaubt
            goto UNDO;
         }

         // aktueller Halbzug-Index
         int uj = (int)(movecount * 2) - 1;

         if(uj < 2) uj = 2;              // absolute Untergrenze: Startstellung
         if(uj >= MaxMvh) uj = MaxMvh-1;

         // Board + Turn exakt restaurieren
         memcpy(board, mvh[uj].board, sizeof(board));
         turn = mvh[uj].turn;

         // Zugzähler exakt auf diesen Halbzug setzen
         movecount    = ((float)uj) / 2.0;
         oldmovecount = movecount;

         // History logisch kürzen
         MaxMvh = uj + 1;
      ...
      else
      {
         ...
         if(strlen(cstring) > 0) {
            if(strlen(cstring) < 4) {
               strcpy(cstring, "a8a8");   // Dummy bei Fehleingabe
            }
            if(strlen(cstring) >= 4) {
         ...
         ChessAIStartMillis = millis();

         //-------------------------------------------------
         score = Minimax(-I, I, Q, O, 1, 3);
         //-------------------------------------------------
      // } <<< muss weg !!!

// es folgt: LegalMove = (turn != oldturn) && (mfrom != mto); // Fix C
...
//-------------------- Anfang  Teil 6 -----------------------
         ...
         LegalMove = (turn != oldturn) && (mfrom != mto); // Fix C
         //fillRect(x, y, w, h, color):
         display.fillRect(0,280, 140,40, BLACK); //2 Zeilen ganz unten, linkes Drittel
         display.setCursor(0,280);
         ...
         if( LegalMove )
         {
            ...
            //vvvvvvvv  testweise AUSKOMMENTIERT  vvvvvvvv
            ...
            // besser evt.: ?
            // RemP = (mto==savEPsq) ? savto : (oldboard[mto]) ? mto : S;

            spiece=psymbol[board[mto]&15];
            if(spiece=='*' || spiece=='+' || spiece=='P' || spiece=='p') {
               spiece=' ';
            }
            char sb='-';  // Trennzeichen: from-to vs. fromXto
            ...
            MaxMvh+=1;  // erweitere mvh heap +1
            mvh = (MvHist*)realloc( mvh, MaxMvh * sizeof(MvHist) );

            j=( (int)(movecount*2) );  // mvh-Index 1.0->2, 1.5->3,..
            if(j>=MaxMvh-1) j=MaxMvh-1; // derzeit redundant, aber korrekt+defensiv
            ...
            mvh[j].sto[2]=0;
            mvh[j].sb[0]=sb;  //  sfrom-sto  vs. sfromXsto
            mvh[j].sb[1]=0;

            // --------- ERWEITERUNG: Undo-Daten im mvh sichern ---------
            memcpy(mvh[j].board, oldboard, sizeof(board));
            mvh[j].turn = oldturn;
            // ----------------------------------------------------------
...
QUIT_CHESS:
   ...
   display.setTextColor(RED);
   display.fillRect(0,280, 420,40, BLACK); // untere 2 Zeilen, volle Breite
...
void showAppsMenu() {
   ...
   if(MENULEVEL==1) {
      if(newMenuLevel) {
         ...
         display.setTextColor(LIME);    display.println();
         display.print  ("  (3: Le)");
         display.setTextColor(RED);
   ...
   if (MENULEVEL==2) {
      ...
      if(newMenuLevel) {
         ...
         // btn 0 ESC
         if (btn00==1 &&  cursormarknr>=0) {      // AD-Button: ESC
            cursormarknr=-1;
            ls(filelist);
            btn00=0;
         }
         else if (btn00==1 &&  cursormarknr<0) {      // AD-Button: ESC
            MENULEVEL = 0;
            newMenuLevel=true;
            oldwinmin=-2;
            display.fillScreen(COLOR_BGND);
   ...
   if (MENULEVEL==3)
   {
      ...
      if(newMenuLevel) {
         ...
         time_refreshDisplay();
         read_ADS0();
         if (btn00==1) newMenuLevel=0;     // AD-Button: ESC
      }
      if(!newMenuLevel) {
         if (btn00==1) {    // AD-Button: ESC
            MENULEVEL = 0;
   ...
   if(MENULEVEL==4) {
      if(newMenuLevel) {
         ...
         while(btn00!=1 && btn00!=3) {
            read_ADS0();
            if(btn00==1) return;    // AD-Button: ESC
            delay(100);
         }
         Chess();  // <<<<<<<<<<<<<<<<<   CHESS !
      }
...
void setup() {
   ...
   TSbutton5.initButton(&display, display.width()-30,280,  60,28,  CYAN, LIGHTBLUE, YELLOW, "...", 2);
...
#define MY_TZ "CET-1CEST,M3.5.0/02,M10.5.0/03" // Europe/Berlin  CT-1CEST,M3.5.0,M10.5.0/3
   ...
   */
   ...
   Serial.println();
   Serial.println("randomseed(): done!\n");
