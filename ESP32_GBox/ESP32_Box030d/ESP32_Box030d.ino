...
void markPos(int old, int cnt) {
   ...
   display.fillRect ( 0, 20*(old-lsstartnum), 14, 14, COLOR_BGND); // <<<< filepos
   display.setTextColor(LIME);
   //display.setCursor( 0+240*(cnt%2), 0+12*(cnt/2));
   display.setCursor( 0, 0+20*(cnt-lsstartnum) );
   display.print(">");
...
void ls(std::vector<String> list) {
   int cnt=0, cursornr=cursorfilenr;
   lsstartnum=0;
   cursornr=max(0,cursorfilenr);
   display.fillScreen(BLACK);
   cursornr>=14? (lsstartnum=cursornr-12) : (lsstartnum=0);
   Serial.println("print list[]:");
   for (cnt = lsstartnum; cnt < filecount; cnt++) {
      ...
      display.setCursor(20,  20*(cnt-lsstartnum) );  // <<<< filepos
...
int32_t btnstate00(int val) {
   if (isInRange( adc00,  PAD_NONE, 1020 ) ) return 0; // NONE
   if (isInRange( adc00,  PAD_ESC,   400 ) ) return 1; // ESC
   if (isInRange( adc00,  PAD_UP,    200 ) ) return 2; // up
   if (isInRange( adc00,  PAD_LEFT,   40 ) ) return 3; // left
   if (isInRange( adc00,  PAD_RIGHT,  90 ) ) return 4; // right
   if (isInRange( adc00,  PAD_DOWN, 1024 ) ) return 5; // down
...
UNDO:
   ...
   while(CHESSACTIVE && CHESSFIGHT) {
      ...
      if(tsBtnPressed==1) { // TsBtnUp=Restart
         tsBtnPressed=-1;
         strcpy(cstring,"R");
         score=I;
         i=1;
      ...
      if(tsBtnPressed==2) { // TsBtnDRight=UNDO <<<<<<<<<<<<<<<<UUUUU
         tsBtnPressed=-1;
         strcpy(cstring,"U");
         score=I;
         i=1;
      ...
      if(tsBtnPressed==4) { // TsBtnDown=auto
         tsBtnPressed=-1;
         strcpy(cstring,"");
         score=I;
         goto EVAL_STRING;
      ...
      if(x>=xoffs-20 && y>=yoffs && x<xoffs+237  && y<yoffs+237 && z>30 ) {
         TSMOVESTATE=1;
         strcpy(pfrom, "");
         strcpy(pto, "");
         strcpy(cstring, "");
...
bool Explorer() {
   ...
   if (btn00==5
      ...
      if (cursorfilenr < filecount-1) {
         ...
         cursorfilenr++;
         if(cursorfilenr>=14) ls(filelist);
      }
   }
   ...
   // btn up 2
      ...
      if(cursorfilenr>12) ls(filelist);
   }
...
void showAppsMenu() {
   ...
   if (MENULEVEL==2) {
      ...
      if(newMenuLevel) {
         ...
         // btn 0 ESC
         if (btn00==1 &&  cursorfilenr>=0) {
            cursorfilenr=-1;
            ls(filelist);
            btn00=0;
         }
         else if (btn00==1 &&  cursorfilenr<0) {
            MENULEVEL = 0;
            newMenuLevel=true;
            display.fillScreen(COLOR_BGND);
            display.setFont();
