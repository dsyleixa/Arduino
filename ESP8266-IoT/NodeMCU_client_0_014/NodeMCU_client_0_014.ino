...
//---------------------------------------------------------------------------- 
// PCF8574 (8* digital IO)  0x20

#include <PCF8574.h>
#define   PCF8574addr  0x20
PCF8574   pcf20(PCF8574addr);
...
char * dashboard(int mode) {
  ...
  if(mode==0) {   
       ...
       display.setCursor( 0,16);  display.print("1." + (String)c0t1.sact + STR_DEGREE + "C");       
          display.setCursor(75,16);  display.print("F:"+(String)c0h1.sact );
       display.setCursor( 0,24);  display.print("2." + (String)c0t2.sact + STR_DEGREE + "C" );       
  ...
  if (mode == 2) {    display.setFont(&FreeSansBold12pt7b);    
    display.setCursor( 0, 20);  display.print("T Aussen:");
    display.setCursor( 0, 40);  display.print( (String)c0t1.sact + STR_DEGREE + "*C");
    display.display();
  }
  else
  if (mode == 3) {
  ...
  if (mode == 4) {
    ...
    display.setCursor( 0, 40);  display.print( (String)c0t2.sact + STR_DEGREE + "*C");
    display.display();
  }
  else
  if (mode == 5) {
...
void setup() {
  ...
  // i2c: PCF8574
  pcf20.begin();
  pcf20.write8(0xFF); // (all pins INPUT HIGH)
  delay(1);
  ...
  //----------------------------------------
  // OLED64x48: Initialising the UI will init the displayBRK too.
  // displayBRK.init();
...
void loop() {     
   ...
   String Sargval=""; 
   char carg[MTKLEN], ctok[MTKLEN], cmsg[MAXLEN];    
   Sargval="";
   ...
   if(cmsg!="") {         
        cstringarg(carg, cmsg, "c0out1");  // out1  switch on/off/rev
        Sargval=(String)carg;
        if(Sargval!="") {
           c0out1=Sargval.toInt(); 
        }   

        cstringarg(carg, cmsg, "c0out2");  // out2  switch on/off
        Sargval=(String)carg;
        if(Sargval!="") {
           c0out2=Sargval.toInt(); 
        }       

        cstringarg(carg, cmsg, "c0out3");  // out3  switch on/off
        Sargval=(String)carg;
        if(Sargval!="") {
