 ...
 *  Client 1, Port 8081, wifi server .200
 *   
 * 
 *  Quelle website:
 ...
 */
 ...
 char * clientname = "Client 1"; // Küche  
 char * ver = "1.016a";
...
static vlog c1t1, c1h1, 
            c1t2, c1h2,             // local temperature, humidity 
            c1p1, c1q1, c1espA0;    // local barometr.air pressure, quality 
...
#define PIN_DHT1    D5       //  DHT Freezer Sensor DHT22 <<<<<<<<<<<<<<
#define PIN_DHT2    D7       //  DHT Cooler Sensor DHT22  <<<<<<<<<<<<<<

#include <DHT.h> 
// DHT sensor defs
 
DHT  DHT_1(PIN_DHT1, DHT22),  // 1.DHT Typ definieren <<<<<<<<<<<<<<
     DHT_2(PIN_DHT2, DHT22);  // 2.DHT Typ definieren <<<<<<<<<<<<<<
...
const char* hostIP = "192.168.2.200"; // Server der die temperatur empfangen soll 
const int httpPort = 8081;  
const char* script = "/client/client2/"; // URL/Verzeichnis das wir gewaehlt haben

//const char* idnr = "2"; //Hier kann man dem Sensor eine beliebe ID zuteilen (derzeit ungenutzt)
...
char * dashboard(int mode) {
  ...
  if(mode==1) {   
       ...
       //display.setFont();
       display.setCursor( 0, 0);  display.print("timeout svr="+String(timeoutcnt) );   
       display.setFont(&FreeMono8pt7b);           
       display.setCursor( 0,20);  display.print("Cooler " + (String)c1t2.sact +  "*C");   // <<<<<<<<<<<    
       display.setCursor( 0,32);  display.print("Freezr " + (String)c1t1.sact +  "*C" );  // <<<<<<<<<<<
       display.setCursor( 0,44);  display.print("Smoke  " + (String)c1espA0.sact + "%" );          
...
void loop() {  
   ...
   ftmp = analogRead(A0);                   // int A0 auslesen
     delay(1);
   ...
   */
   ...
   do
   {
     ...
     erg =client.connect(hostIP, httpPort);
     if (!erg) {
       versuche++; 
       timeoutcnt++;
       dashboard(1);
   ...
   c1out2=0;
   if(cmsg!="") {         
        cstringarg(cmsg, "c1out0", carg);  // alert pin (D6) switch on/off
        Sargval=(String)carg;
        if(Sargval!="") {
           c1out0=Sargval.toInt(); 
        }   

        cstringarg(cmsg, "c1out1", carg);  // switch on/off
        Sargval=(String)carg;
        if(Sargval!="") {
           c1out1=Sargval.toInt(); 
        }  

        cstringarg(cmsg, "c1out2", carg);  // switch on/off
        Sargval=(String)carg;
        if(Sargval!="") {
   ...
   if(c1out0>0 || emergencycnt>0 ) {           // Server message: alarm, LED ON, please confirm global alive
      digitalWrite( PIN_OUT0, HIGH);
   }
   else if(c1out0==0 && emergencycnt==0 ) {    // Server message: no alarm, LED OFF, global still alive
      digitalWrite( PIN_OUT0, LOW);
   }
   if(c1out2==0) digitalWrite(PIN_OUT2, LOW); else digitalWrite(PIN_OUT2, HIGH);
   
   //  if(c1out3==0) digitalWrite(PIN_OUT3, LOW); else digitalWrite(PIN_O3, HIGH);
