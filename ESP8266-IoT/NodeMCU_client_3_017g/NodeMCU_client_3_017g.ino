...
/*
  ...
  motorshield 
   ...
   D0     16       WAKE           ---           ---               ---
   D1      5       I2C SCL        I2C SCL       I2C SCL           out D1 pwmA / in DHT
   D2      4       I2C SDA        I2C SDA       I2C SDA           out D2 pwmB
   D3      0       FLASH/LED      out D3     built-in btn+LED     out D3 motorA
   D4      2       TX1            In DHT        ---               out D4 motorB
   D5     14       SPI SCK        In B/S        out D5            I2C SCL
   D6     12       SPI MISO       Out D6        ---               I2C SDA
...
// change log
// 3.017d OLED disp.opt. + A0intern als vlog  
// 3.017d OLED disp.opt.
// 3.017c fINVAL = -999.0; ADS:0x48; Aussen
...
#include <Wire.h>
   ...
   #define ESPSCL   D5    // esp-L293 Board vertauscht
   #define ESPSDA   D6 
...
// esp8266 morotrshield:
#define PIN_P1    D1     // pwmA  <<< intern für Thermostat >> Ventilator c3out3
#define PIN_O1    D3     // dirA
#define PIN_P2      D2     // pwmB

#define PIN_O2      D4     // dirB

// output pins (LED, motors)
#define PIN_P3    D8     // pwmC opt.


/*
BTS7960-Modul sind mit 3,3 V kompatibel
Pins In1 und In2 die RPWM und LPWM Pins sind, die Sie für die Steuerung benötigen. 
Der EN-Pin aktiviert beide Seiten des Treibers gleichzeitig.
ER02/ER01 =  L_IS / R_IS Error/Current Sense Output: Ausgänge für die Strommessung 
In2 / In1   LPWM / RPWM Inputs/PWM:  Hauptsteuerpins. In1 entspricht RPWM und In2 entspricht LPWM
EN  = L_EN & R_EN Enable:  scheint intern Enable-Pins (L_EN, R_EN) kurzgeschlossen zu haben. 
nur 1 Pin mit 5V/3v3 auf HIGH setzen, um das gesamte Modul zu aktivieren.
#define PIN_P1    D1     // pwmA <<< intern für Thermostat >> Ventilator 
#define PIN_P2    D2     // pwmB
#define PIN_EN      D3   // nur 1x enable für beide H-Bridges gleichzeitig
//   D4 frei
#define PIN_P3    D8     // pwmC opt.
*/
//----------------------------------------------------------------------------
// digital 1-wire pins for DHT sensors
#define PIN_DHT2    D7       //  DHT Sensor 2 STANDARD innen => c3t2+c3h2
// #define PIN_DHT1    D4?       //  DHT Sensor 1 VARIANT außen => c3t1+c3h1
...
DHT  DHT_2(PIN_DHT2, DHT22);  //   DHT22 Sensor STANDARD innen => c3t2+c3h2
//DHT  DHT_1(PIN_DHT1, DHT22);  //  anderer DHT Typ

float c3tx3=fINVAL;
float TEMP_THERM=24.0;
...
static vlog c3t1, c3h1, c3t2, c3h2,  // local temperature, humidity
       c3p1, c3q1, c3espA0;              // local barometr.air pressure, air quality
...
const char* urldir = "/client/client3/"; // URL/Verzeichnis das wir gewaehlt haben

const char* idnr = "3"; //Hier kann man dem Sensor eine beliebe ID zuteilen (derzeit ungenutzt)
...
char * dashboard() {
   ...
   if(DisplayMode==0) {
      ...
      display.setCursor( 0,32);  display.print("3.iA0="+(String)c3espA0.sact);
      ...
      display.setCursor( 0, 20);  display.print((String)"Name "+clientname);
      display.setCursor( 0, 39);  display.print((String)"Vers " + ver);
      display.display();
      lcd.setCursor(0,0); lcd.print((String)"Name " + clientname);
      lcd.setCursor(0,1); lcd.print((String)"Vers " + ver);
      ...
      display.setCursor( 0, 20);  display.print("T+H Auss.");
      display.setCursor( 0, 39);  display.print( (String)c3t1.sact + " *C " );
      display.setCursor( 0, 58);  display.print( (String)c3h1.sact + " %" );
      display.display();
      lcd.setCursor(0,0); lcd.print("T+H Auss.");
      ...
      display.setCursor( 0, 39);  display.print( (String)c3t2.sact + " *C " );
      display.setCursor( 0, 58);  display.print( (String)c3h2.sact + " %" );
      display.display();
      ...
      display.setFont(&FreeMono9pt7b);
      display.setCursor( 0, 20);  display.print("AirQ: "); display.print((String)(int)c3espA0.vact+"%");
      display.setCursor( 0, 39);  display.print("Therm "); display.print((String)(int)TEMP_THERM + "*C");
      display.setCursor( 0, 58);  display.print("Mot 3 "); display.print((int)c3out3==0?"OFF":"ON");
      display.display();
      lcd.setCursor(0,0); lcd.print("Ai Air-Q: ");  lcd.print((String)(int)c3espA0.vact+"%");
      lcd.setCursor(0,1); lcd.print("Thermost: ");  lcd.print((String)(int)TEMP_THERM + "°C");
      lcd.setCursor(0,2); lcd.print("MotOut3 : ");  lcd.print((int)c3out3==0?"OFF":"ON");
      lcd.setCursor(0,3); lcd.print("");
      ...
      display.setFont(&FreeMono9pt7b);
      display.setCursor( 0, 15);  display.print("A0: ");  display.print(sfadc0);
      display.setCursor( 0, 31);  display.print("A1: ");  display.print(sfadc1);
      display.setCursor( 0, 47);  display.print("A2: ");  display.print(sfadc2);
      display.setCursor( 0, 63);  display.print("A3: ");  display.print(sfadc3);
      display.display();
      lcd.setCursor(0,0); lcd.print("A0: "); lcd.print(sfadc0);
      lcd.setCursor(0,1); lcd.print("A1: "); lcd.print(sfadc1);
      lcd.setCursor(0,2); lcd.print("A2: "); lcd.print(sfadc2);
      lcd.setCursor(0,3); lcd.print("A3: "); lcd.print(sfadc3);
...
void setup() {
   ...
   ADSx48.begin(0x48); // ADS1115 I2C 4x 12/16bit ADC ads0.begin(0x48)
   delay(1);
...
void loop() {
   ...
   ftmp = DHT_2.readTemperature();          //  Temperatur auslesen (Celsius)
   delay(10);
   if (isnan(ftmp)) ftmp=fINVAL;
   logval(ftmp, c3t2);         //  DHT Sensor innen => c3t2+c3h2
   delay(10);

   ftmp = DHT_2.readHumidity();             //  Feuchtigkeit auslesen (Prozent)
   delay(1);
   if (isnan(ftmp)) ftmp=fINVAL;
   logval(ftmp, c3h2);
   delay(10);
   ...
   /*
   ftmp = DHT_1.readTemperature();          //   Temperatur auslesen (Celsius)
   delay(10);
   if (isnan(ftmp)) ftmp=fINVAL;
   logval(ftmp, c3t1);
   delay(10);
   ftmp = DHT_1.readHumidity();             //   Feuchtigkeit auslesen (Prozent)
   delay(1);
   if (isnan(ftmp)) ftmp=fINVAL;
   logval(ftmp, c3h1);
   delay(10);
   */
   ...
   /*
      ...
      if (isnan(ftmp)) ftmp=fINVAL;
      logval(ftmp, c3t1);      // bme temperature => T1 außen 
      delay(1);

      ftmp=fINVAL;
      ftmp = bme_x77.readHumidity();       // 2. Feuchtigkeit auslesen (Prozent)
      if (isnan(ftmp)) ftmp=fINVAL;
      logval(ftmp, c3h1);    // bme temperature => HT1 außen 
      delay(1);

      ftmp=fINVAL;
      ftmp = FNNcorr + bme_x77.readPressure()/100.0 ;
      if (isnan(ftmp)) ftmp=fINVAL;
      logval(ftmp, c3p1);DHT 1 innen:
      delay(1);
   */
   Serial.print("DHT 2 innen: ");
   Serial.print(c3t2.sact);   Serial.print("   ");  Serial.print(c3h2.sact); Serial.println(" rH");
   /*
   Serial.print("DHT 1: ");
   Serial.print(c3t1.sact);   Serial.print("   ");  Serial.print(c3h2.sact); Serial.println(" rH");
   */
   ...
   // internal ADC A0
   // ftmp = analogRead(A0);  //  for extra AirQuality sensor
   ftmp = 1024/2;      
     delay(1);
     if (isnan(ftmp)) ftmp=fINVAL;   
     else ftmp=calADC1023(ftmp);    
     logval(ftmp, c3espA0); 
     delay(1);   
    Serial.print("c3espA0="); Serial.println(c3espA0.sact); Serial.println(); 
   ...
   //---------------------------------------
   if( (int)c3t2.vact >= (int)TEMP_THERM) {      
      c3out3=1;
   }
   else if(c3t2.vact<=TEMP_THERM-2.0) {
      c3out3=0;
   }
   ...
   // esp ADC
   vals += "&c3espA0="; vals += c3espA0.sact;
   ...
   String url = urldir; //Url wird zusammengebaut: urldir = "/client/client3/";
   url += "?pw=";
   url += password;
   url += "&idnr=";
   url += idnr;
   ...
   if(client.available()) {
      ...
      if(cmsg!="") {
         ...
         */
         c3tx3=fINVAL;
         cstringarg(cmsg, "c3tx3", carg);  // Thermostat-Temperatur  switch on/off
         Sargval=(String)carg;
         if(Sargval!="") {
            c3tx3=Sargval.toInt();
         }
         if(c3tx3!=fINVAL) {
            TEMP_THERM = (float)c3tx3;   
         }
      }
      ...
      */
      ...
      if(c3out3==1)
         { digitalWrite(PIN_P1, HIGH); digitalWrite(PIN_O1, HIGH); }
      else if(c3out3==0)
         { digitalWrite(PIN_P1, LOW);  digitalWrite(PIN_O1, LOW); }
