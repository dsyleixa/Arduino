 ...
 *  Client 2, Port 8081, wii server .200
 *  (bRk)
 * 
 *  Quelle website:
 ...
 */
 ...
 /*
  ...
  digital:         default        myStd         bRk               ESP_motorShield
   ...
   D5     14       SPI SCK        In B/S        in D5 (DHT)       SDA
   D6     12       SPI MISO       Out D6        out D6            SCL
   D7     13       SPI MOSI       Out D7        in D7 (DHT)       in  D7 
 ...
 */
 ...
 char * clientname = "Client 2"; // Küche bRk
 char * ver = "0.1.4.0";
...
static vlog c2t1, c2h1, c2t2, c2h2,  // local temperature, humidity 
...
int      c2btn0=0;       // Btn state of PIN_RESETAlarm 
int8     LocAlive=0;
...
char * dashboard(int mode) {
  ...
  if(mode==1) {   
       ...
       display.setCursor( 0,26);  display.print("1." + (String)c2t1.sact + STR_DEGREE + "*C");       
          //display.setCursor(75,26);  display.print("F:"+(String)c2h1.sact );
       display.setCursor( 0,39);  display.print("2." + (String)c2t2.sact + STR_DEGREE + "*C" );       
...
//----------------------------------------------------------------------------
 void checkRESETBtn(){
    c2btn0=digitalRead(PIN_RESETAlarm);              // btn press bRk built-in Btn 0=D3
    if(LocAlive==0 && !c2btn0)  LocAlive=1;    // store local alive=true when btn press (inverse)! 
    delay(1); 
 }
...
void loop() {  
   ...
   */
   ...
   c2out2=0;
   if(cmsg!="") {         
        cstringarg(carg, cmsg, "c2out0");  // alert pin (D6) switch on/off
        Sargval=(String)carg;
        if(Sargval!="") {
           c2out0=Sargval.toInt(); 
        }   

        cstringarg(carg, cmsg, "c2out1");  // switch on/off
        Sargval=(String)carg;
        if(Sargval!="") {
           c2out1=Sargval.toInt(); 
        }  

        cstringarg(carg, cmsg, "c2out2");  // switch on/off
        Sargval=(String)carg;
        if(Sargval!="") {
   ...
   */
   ...
   if(c2out0>0 || alertcnt>0 ) {           // Server message: alarm, LED ON, please confirm global alive
      digitalWrite( PIN_OUT0, HIGH);
   }
   else if(c2out0==0 && alertcnt==0 ) {    // Server message: no alarm, LED OFF, global still alive
      digitalWrite( PIN_OUT0, LOW);
   }
   
   /*
   if(c2out2==0) digitalWrite(PIN_O2, LOW); else digitalWrite(PIN_O2, HIGH);
   if(c2out3==0) digitalWrite(PIN_O3, LOW); else digitalWrite(PIN_O3, HIGH);
   */
