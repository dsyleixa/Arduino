...
// 0.9.d:  ?
// 0.9.c:  handlenotRoot() neu
// 0.9.b:  handlenotAuthorized() neu
// 0.9.b:  handleWebsite() neu
...
// Anzahl aller zu überwachenden Ausgänge (4 Clients x 3 Ausgänge)
const int NUM_OUTPUTS = 12;

// Letzte bekannte Ausgangszustände (0/1)
volatile int8_t lastOutState[NUM_OUTPUTS] = {0};

// Flag, ob für einen Ausgang eine Ghost-Meldung aktiv ist
bool ghostActive[NUM_OUTPUTS] = {false};

// Letzte Ghost-Meldung als String
String lastGhostMsg = "";

// Array für erwartete Änderungen (intern für Tracker)
bool expectedChange[NUM_OUTPUTS] = {false};

// Zeit der letzten Ghost-Meldung
...
void loop() {
   ...
   if ( millis() - tms >= 500 ) {    // refresh data rate 100-500
      tms = millis();
      ...
      */
      ...
      Serial.println("Client sensors+motors:");
...
// --- 090i : handleWebsite() — PART A (bis vor HTML-Ausgabe) ---
   ...
   if (c0out1 == 1) script += "EIN &nbsp; <wbr> <wbr> ";
   else script += "AUS &nbsp; <wbr> <wbr> ";
   ...
   if (c0out2 == 1) script += "EIN &nbsp; <wbr> <wbr> ";
   else script += "AUS &nbsp; <wbr> <wbr> ";
...
void handleClients() {
   ...
   String pth = webserver.uri();
   bool legit = true;  // alle GET-Parameter von Clients akzeptieren
...
/// -----------------------
// Funktion: Zeit aus NTP aktualisieren und tmElements_t setzen
// -----------------------
