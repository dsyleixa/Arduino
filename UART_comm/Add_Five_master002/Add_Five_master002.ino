...
void setup() {
   ...
   // Serial 
   i=115200;
   Serial.begin(i);  //Begin Serial to talk to the Serial Monitor   
   Serial1.begin(i); //Begin Serial to talk to the Rx Arduino
...
void loop()
{
  ...
  while (incoming != 0)           //while there is something to be read
  {
    ...
    if (val != 0)
    {
      ...
      sprintf(sbuf, "transm.: %5d", val);
      lcdprintxy(0,10, sbuf);
