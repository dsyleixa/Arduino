...
uint8_t  val[]={bsync,chksum,2,3,4,5,6,7};
...
void loop()
{
  ...
  for(cnt=0; cnt<bwidth; ++cnt) {
     ...
     lcdprintxy(cnt*3*8,20, sbuf);  // print on TFT 
  ...
  // display  values:
  lcdprintxy(0,30, "Receiving...");  
  Serial.println("Receiving...");
  
  if(inval[0]==bsync) 
  {
     for(cnt=0; cnt<bwidth; ++cnt) {
       sprintf(sbuf, "%3d  ", inval[cnt]);      // print on TFT
