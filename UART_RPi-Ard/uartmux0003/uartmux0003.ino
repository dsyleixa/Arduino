/*       
       ...
       log:
       GPS ublox thread: Lat, Long-> sendarray
...
double          fLati=-9999.0, fLong=-9999.0;
float           fspeed=-9999.0, fcourse=-9999.0;
int16_t         dday=-9999, dmonth=-9999, dyear=-9999, 
                dhour=-9999, dmin=-9999, dsec=-9999, dcsec=-9999, nsat=0;
//=====================================================================================
...
void buildSendArray() {  
  ...
  DoubleToByteArray(fLati, sendbuf, DOUBL0);    // store double Latitude
  DoubleToByteArray(fLong, sendbuf, DOUBL1);    // store double Longitude 
...
{ 
  ...
  sprintf(sbuf, "recieve: %4d %4d   send: %4d %4d La %13.8f Lo %13.8f", 
                 recvbuf[4], recvbuf[6], sendbuf[4], sendbuf[6], fLati, fLong);
  Serial.println(sbuf); 
...
void setup() {
   ...
   Serial.println("Serial started: ");  Serial.println(SERIALbaud);
...
{  
  ...
  { 
    fLati= (double)gps.location.lat();
    fLong= (double)gps.location.lng(); 
    sprintf(sbuf, "Latt: %+013.8f , Long: %+013.8f ", fLati, fLong ); 
    Serial.print(sbuf);
  }
  else { fLati=-999.0; fLong=-999.0; Serial.print(" Location:  INVALID      ");}

  //----------------------------
  if (gps.speed.isValid())
  { 
    fspeed= (float)gps.speed.kmph();
  ...
  if (gps.time.isValid())
  {
    ...
    sprintf(sbuf, " Time: %02d:%02d:%02d Satellites=% 2d ", dhour, dmin, dsec, nsat); 
    Serial.print(sbuf);
  }
  else { Serial.print(" Time+SatSignal: INVALID     "); }
