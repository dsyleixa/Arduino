...
void displayvalues(uint8_t array[]) {
...
{ 
  ...
  resOK = receive ( cbuf, 10000,&cnt);          // receive + store to temp buffer
  ...
  if( resOK ) {                                 // receive ok?
    ...
    //displayvalues(cbuf);
     ...
     memcpy(recvbuf, cbuf, sizeof(cbuf));       // copy valid temp buffer to valid recieve buf  
     memset(sendbuf, 0, sizeof(sendbuf));       // clear send buf before use
     
                                  // debug: change test values to send back!        
  ...
  for(uint8_t i=0; i<MSGSIZE; i++) {       
     Serial.write(sendbuf[i]);                      // Send value to master       
  }       

  //displayvalues(sendbuf);
...
void setup() {
   ...
   Serial.begin(UARTclock);                    // RX-TX UART
   while(Serial.available())  Serial.read();   // clear output buffer from chunk
