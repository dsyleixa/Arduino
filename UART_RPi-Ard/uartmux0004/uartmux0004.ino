/*
       ...
       log:
       course, speed, nsat -> sendarray
       time, date, nsat: int8_t
       INVALID: -1001
...
int8_t         dhour = INVALCH, dmin = INVALCH, dsec = INVALCH, dcsec = INVALCH, nsat = 0;
...
void buildSendArray() {
  ...
  DoubleToByteArray(fLat, sendbuf, DOUBL0);    // store double Latitude
  DoubleToByteArray(fLng, sendbuf, DOUBL1);    // store double Longitude
  Int16ToByteArray(dcoursex32, sendbuf, ANA7); // GPS course*32
  Int16ToByteArray(dspeedx32, sendbuf, ANA8);  // GPS speed*32
  sendbuf[BYTE2] = nsat;                       // GPS n satellites
...
void loopUARTcomm ()
{
  ...
  sprintf(sbuf, "recieve: %4d %4d   send: %4d %4d La %13.8f Lo %13.8f  %6.1f  %4.1f  %2d",
          recvbuf[BYTE0],recvbuf[BYTE1],sendbuf[BYTE0],sendbuf[BYTE1],fLat,fLng,fcourse,fspeed,nsat);
  Serial.println(sbuf);
...
void getGPSvalues()
{
  ...
  if (gps.location.isValid())
  {
    ...
    fLng = (double)gps.location.lng();
    sprintf(sbuf, "Lat: %+014.9f , Lng: %+014.9f ", fLat, fLng );
    Serial.print(sbuf);
  }
  else {
    fLat = INVALID;
  ...
  dspeedx32 = (int16_t)(fspeed * 32.0); // *32
  ...
  dcoursex32 = (int16_t)(fcourse * 32.0); // *32
  ...
  if (gps.date.isValid())
  {
    ...
    dday = INVALCH; dmonth = INVALCH; dyear = INVALCH;
  ...
  if (gps.time.isValid())
  {
    ...
    nsat = (int8_t)gps.satellites.value();
    sprintf(sbuf, " Time: %02d:%02d:%02d Satel.=% 2d ", dhour, dmin, dsec, nsat);
    Serial.print(sbuf);
  }
  else {
    dhour = INVALCH; dmin = INVALCH; dsec = INVALCH; nsat = INVALCH;
    Serial.print(" Time+SatSignal: INVALID     ");
  }
